locals {
  ...
  c4_project_id                                 = data.terraform_remote_state.env.outputs.prj_s_app_c4
  smcs_project_id                               = data.terraform_remote_state.env.outputs.prj_s_app_smcs
}
...
data "terraform_remote_state" "env" {
  ...
  config = {
    bucket = var.remote_state_bucket
    prefix = "terraform/environments-new/stage"
  }
}
