locals {
  ...
  it_project_id                                 = data.terraform_remote_state.env.outputs.prj_it_app_IT
  mark_project_id                               = data.terraform_remote_state.env.outputs.prj_it_app_Marketing
}
...
data "terraform_remote_state" "env" {
  ...
  config = {
    bucket = var.remote_state_bucket
    prefix = "terraform/environments-new/it_infra"
  }
}
